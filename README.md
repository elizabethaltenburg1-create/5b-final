...
Simulates importing GoToWebinar registration/attendance data (mock
data — see `lib/mockImport.ts`) for completed webinars that don't have
registrations yet.
Scores every registration that doesn't have a lead yet using the fixed
...
```bash
   ...
   ```
Run the migration in your Supabase project's SQL Editor (see Data model
above).
...
```bash
   ...
   npm run dev
   ```
Open http://localhost:3000 — you should see the sample completed webinar.
...
In the Vercel project's Environment Variables settings, add all six
variables from the table above.
Deploy. `vercel.json` registers the daily cron job automatically
(`/api/cron/import-and-score`, currently scheduled for 13:00 UTC — edit
the `schedule` field to change it).
Manually trigger the cron once after deploying to confirm it's wired up:
```bash
